...
const protectedRoute = catchError(async (req, res, next) => {
    ...
    if (!decode?.userId) return next(new AppError(messages.token.invalidToken, 401));
    ...
    if (user.passwordChangedAt) {
        ...
        if (time > decode.iat) return next(new AppError(messages.token.invalidToken + " " + "Login agin ", 401))

    };

    /* Check If User Login or no */

    if (user.status == status.OFFLINE) return next(new AppError(messages.user.mustLogin, 401))
